...
export const createClientLogger = (props: LoggerProps) => {
    ...
    if (process.env.NODE_ENV !== "test" && props.remoteLoggingProps) {
        ...
        const remoteLogConnectionProps = {
            query: {
                clientInfo: JSON.stringify(props.remoteLoggingProps.systemIdentifiers),
                appID: props.remoteLoggingProps.appID,
            }, // Send client info during connection
            autoConnect: true,
    ...
    function logDecorator(name: string, color: keyof typeof colors, logToConsole = true, logLevel: LogLevel = "Debug") {
